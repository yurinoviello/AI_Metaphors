...
from ai_metaphors.core.utils.text_utils import extract_code_from_html, extract_steps_from_js

class CodePromptProvider(PromptProvider, ABC):

    _term: dict[str, str]

    _PATH_TO_CODE: Path = Path("ai_metaphors/video_from_code/prompts")
    ...
    def _get_input_data(self) -> str:
        return self._INPUT_DATA.read_text().format_map(
            SafeDict(
                ...
                code_block=extract_code_from_html(Path(self._term["code_folder"]) / "index.html"),
                steps=extract_steps_from_js(Path(self._term["code_folder"]) / "script.js")
            )
        )
