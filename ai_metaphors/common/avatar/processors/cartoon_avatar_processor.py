...
class CartoonAvatarProcessor:
    ...
    def generate_video_with_avatar(self):
        animation = animate(audio_file=self._audio_path, transcript=self._get_transcript())
        background_clip = VideoFileClip(self._video_path).with_fps(self._fps).with_duration(animation.duration)
        animation.export(path=self._output_path, background=background_clip, scale=0.4)
