...
def animate_term(
    ...
    if manim_type == ManimType.AVATAR:
        narration_text = get_narration_text(manim_code)
        AvatarProvider(
...
def get_narration_text(code: str) -> list[str]: # TODO: possible to replace with a generated by LLM
    match = re.search(r'```python(.*?)```', code, re.DOTALL)
    if not match:
        return []
    clean_code = match.group(1).strip()
    texts = []
    for node in ast.walk(ast.parse(clean_code)):
